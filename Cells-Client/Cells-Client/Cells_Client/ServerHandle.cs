...
namespace Cells
{
    ...
    public class ServerHandle
    {
                ...
                case PacketTypes.SENDPLAYERS:
                    ...
                    if (Game1.map != null && Game1.map.cells.Count > 0)
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            else { Player me = new Player(new Microsoft.Xna.Framework.Vector2(x, y), size, color,PlayerId); Game1.map.players.Insert(0, me); }
                        } break;
        ...
        public void createPlayer(object obj)
        {
            ...
            bool contains = false;
            for (int i = 1; i < Game1.map.players.Count; i++)
        ...
        {
            NetOutgoingMessage msg = client.CreateMessage();
            msg.Write((byte)PacketTypes.MOUSE);
            msg.Write(x);
            msg.Write(y);
            client.SendMessage(msg, NetDeliveryMethod.UnreliableSequenced, 8);
        }
